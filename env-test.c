...
// ftw flags
static int FTW_FLAGS = FTW_PHYS | FTW_DEPTH;  // if the dir structure is large, this will take some time, since we limit the number of open files
...
static int
_remove(const char *fpath, const struct stat *sb,
    ...
    if (tflag == FTW_DP) {
        ...
        slurm_debug3("private-tmpdir: removing symlink: %s", fpath);
        rc = unlink(fpath);
    }
    else if (tflag == FTW_F) {
        slurm_debug3("private-tmpdir: removing symlink: %s", fpath);
